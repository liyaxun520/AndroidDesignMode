...
class LoveView(context: Context) : RelativeLayout(context) {
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        //用这个来判断是否是2击事件，判断数组中pos=2的点击事件的时间与数组中pos=0的点击事件的时间差值是否小于500，若是小于500认为是2击事件，这时需要绘制爱心图片
        if (mHits[0] >= (SystemClock.uptimeMillis() - 500)) {
            ...
            //设置图片资源
            iv.setImageDrawable(resources.getDrawable(R.drawable.icon_home_like_after))
            iv.layoutParams = lp
            ...
            animatorSet.play(
                ...
                //旋转动画，随机旋转角
                .with(rotation(iv, 0, 0, num[(0..4).random()]))
                //渐变透明动画，透明度从0-1
